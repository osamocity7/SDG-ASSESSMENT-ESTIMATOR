...
const covid19ImpactEstimator = (data) => {
  ...
  if (data.periodType === 'days') {
    output.impact.infectionsByRequestedTime = Math.trunc((2 ** (Math.trunc(data.timeToElapse / 3)))
    * output.impact.currentlyInfected);
    // console.log((2 ** (Math.trunc(data.timeToElapse / 3))) * output.impact.currentlyInfected);
    output.severeImpact.infectionsByRequestedTime = Math.trunc(output.severeImpact.currentlyInfected
    * (2 ** Math.trunc(data.timeToElapse / 3)));
  } else if (data.periodType === 'months') {
    output.impact.infectionsByRequestedTime = Math.trunc(output.impact.currentlyInfected
